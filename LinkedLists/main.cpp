...
struct node* createNode()
{
    ...
    //In C++, we use NEW() and DELETE() for dynamic memory allocation. You can use MALLOC and FREE also.
    //Instead of the whole MALLOC line. you can write this:   n = new node();
...
void insertNodeAtFront()
{
    ...
    // i think 'temp' is created only because we do not want to disturb the value in 'start' .
    struct node *temp;
    temp = createNode();

    cout<<"Enter a value : \n";  //we can also enter this value as an argument in insertNodeAtFront() .
    cin>>temp->data;
...
void deleteNodeFirst()
{
    ...
    else
    {
        ...
        //If you have used 'new()'  , then you have to write 'delete(r)' here.
     }
